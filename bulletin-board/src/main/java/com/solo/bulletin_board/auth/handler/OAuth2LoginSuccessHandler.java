package com.solo.bulletin_board.auth.handler;
...
public class OAuth2LoginSuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request,
        ...
        if(optionalMember.isPresent()){
            if(optionalMember.get().getSignupType() == Member.SignupType.GOOGLE_OAUTH2){
                redirect(request, response, optionalMember.get());
            }
            else{
            ...
            Member savedMember = createMember(email, nickname);
    ...
    private void redirectWithError(HttpServletRequest request, HttpServletResponse response) throws IOException{
        ...
        URI uri = UriComponentsBuilder
                ...
                .port(3000)
                .path("/receive")
                .queryParam("error", "This email is already registered using Server")
                .build()
                .toUri();
